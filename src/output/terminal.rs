...
pub fn format_result(result: &ScanResult, no_color: bool) -> String {
    ...
    let depth_info = if result.depth > 0 {
        format!(
            " {}",
            format!("(depth: {})", result.depth).with(overlay_color)
        )
    } else {
        String::new()
    };
    format!(
        "  {} {:>3}  {:>8}  {:>5}w  {}{}{}",
        "●".with(status_color),
        result.status.to_string().with(status_color),
        result.human_size().with(size_color),
        result.words.to_string().with(words_color),
        result.url.clone().with(text_color),
        redirect_info,
        depth_info
    )
}
...
pub fn format_recursion_indicator(path: &str, no_color: bool) -> String {
    if no_color {
        return format!("      ├── scanning recursively: {}", path);
    }
    let teal = colors::rgb(colors::SKY);
    format!(
        "      {}",
        format!("├── scanning recursively: {}", path).with(teal)
    )
}
