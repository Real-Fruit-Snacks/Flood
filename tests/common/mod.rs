...
pub async fn setup_mock_server() -> MockServer {
    ...
    Mock::given(method("GET")).and(path("/api"))
        .respond_with(ResponseTemplate::new(200)
            .set_body_string("{\"status\": \"ok\"}")
            .insert_header("content-type", "application/json"))
        .mount(&server).await;

    Mock::given(method("GET")).and(path("/images"))
    ...
    Mock::given(method("GET")).and(path("/images/"))
        .respond_with(ResponseTemplate::new(200)
            .set_body_string("Directory listing for images"))
        .mount(&server).await;

    Mock::given(method("GET")).and(path("/.htaccess"))
        .respond_with(ResponseTemplate::new(403).set_body_string("Forbidden"))
        .mount(&server).await;

    Mock::given(method("GET")).and(path("/secret"))
...
pub fn create_wordlist(words: &[&str]) -> NamedTempFile {
    ...
    for word in words { writeln!(f, "{}", word).unwrap(); }
    f
}
